...
이 문서의 핵심 전제였던 **"T4 — VTOR·PRIMASK 두 원인이 실제 부팅 경로에서 동시에 존재했고 PRIMASK가 VTOR 증상을 은폐했다"는 해석은 철회한다.** 코드·git 재분석 결과 두 원인은 서로 다른 실행 경로에 속한다:

- **디버거 경로**(부트로더 우회): VTOR만 문제, PRIMASK=0(정상)
- **물리 리셋 경로**(부트로더 경유): PRIMASK만 문제, VTOR은 부트로더가 정상 설정

`SystemInit()`은 `USER_VECT_TAB_ADDRESS`가 줄곧 주석 처리되어 VTOR을 설정한 적이 없으므로, 실제 부팅에서 VTOR은 부트로더가 설정한 `0x08010000`으로 정상이었다. 따라서 실험 B에서 "물리 리셋 경로인데 VTOR=0"으로 관측했다는 기록은 부트로더가 점프 직전 VTOR을 설정한다는 코드와 **모순되며, end-to-end 추적으로 `0xE000ED08`(SCB->VTOR)을 재확인할 필요가 있다.** 다만 각 레지스터(PRIMASK·VTOR)를 단독 주입해 인과를 보인 실험 A·B의 *방법론*과 개별 관측 자체는 유효하다.

상세 근거: [ISS-BL-001 재분석(2026-06-13)](ISS-BL-001_can-irq-after-bootloader.md)
...
~~두 원인이 **동시에** 존재했기 때문에 PRIMASK=1이 VTOR 오류 증상까지 은폐했다.~~ *(정정 2026-06-13: 위 **정정** 절 참조 — 두 원인은 공존하지 않았다. VTOR은 디버거 경로, PRIMASK는 실제 부팅 경로의 각각 단일 원인이다.)*
...
~~T4가 Phase 4/5B에서 실제로 발생한 상태다.~~ → **정정(2026-06-13): 사실이 아님.** 실제 부팅에서 VTOR은 부트로더가 정상 설정했으므로 T4(두 원인 공존)는 발생하지 않았다. 위 **정정** 절 참조.

...
static uint8_t tx_data[8] = {0xDE, 0xAD, 0xBE, 0xEF, 0x00, 0x00, 0x00, 0x00};
...
int main(void)
{
  ...
  while (1)
  {
    ...
    static uint32_t s_drive_tick = 0;
    if (now - s_drive_tick >= 20) {
        s_drive_tick = now;
        drive_update();
    }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : IR_S1_Pin IR_S2_Pin IR_S3_Pin */
  GPIO_InitStruct.Pin = IR_S1_Pin|IR_S2_Pin|IR_S3_Pin;
  ...
  /*Configure GPIO pin : IR_S4_Pin */
  GPIO_InitStruct.Pin = IR_S4_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(IR_S4_GPIO_Port, &GPIO_InitStruct);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    tx_data[2] = (uint8_t)g_driving_state;
    tx_data[3] = (uint8_t)g_obstacle_flag;
    tx_data[4] = (uint8_t)(tx_count >> 24);
    tx_data[5] = (uint8_t)(tx_count >> 16);
    tx_data[6] = (uint8_t)(tx_count >> 8);
    tx_data[7] = (uint8_t)(tx_count);
    tx_count++;

...
| v1.1 | 2026-06-07 | §13 결과 기록 위치를 TR-001 §6.1로 명시, §13.1 집계를 실행 결과(✅16·🔶10·⬜2) 기준으로 갱신 — 계획서/결과서 분리(ASPICE SWE.6) | JaehyeongIm |
| v1.2 | 2026-06-07 | §13.1 집계 갱신(✅19·🔶8·⬜1) — CI/CD E2E 배포 on-target 실증(Jenkins build #85·#86) 반영(TR-001 §4.1·§6.1). TC-CI-001/003/004 ✅ 승격, §13.3 TC-CI-003 완료 체크 | JaehyeongIm |
...
### TC-DRV-004 v2 — 정지 후 자동 후진 복귀 (OTA 후)
...
> **결과 기록 위치:** 본 명세는 *계획*이며, 실제 실행 결과는 **[TR-001 §6.1 계획↔실행 결과 매핑](TR-001_Test_Report.md)** 에 통합·기록한다(ASPICE SWE.6 — 계획서와 결과서 분리). 검증은 호스트 단위(Ceedling)·on-target(SIT-TC-01~08) 두 채널에서 수행됐고, 아래 집계는 TR-001 §6.1과 동일한 범례를 따른다.
> 범례: ✅ 실증 · 🔶 부분·간접(메커니즘 확인, 전용 정량기록 후속) · ⬜ 미실행

### 13.1 결과 집계표 (TR-001 §6.1 기준)

| 카테고리 | 전체 TC | ✅ 실증 | 🔶 부분·간접 | ⬜ 미실행 |
|---------|--------|------|------|-------|
| TC-CAN | 4 | 1 | 3 | 0 |
| TC-DRV | 5 | 1 | 3 | 1 |
| TC-OTA | 8 | 8 | 0 | 0 |
...
- [x] TC-CI-003 E2E 파이프라인 PASS (TR-001 §4.1 — Jenkins build #86)
- [ ] 미해결 High 심각도 결함 없음

...
| 2.8 | 2026-06-03 | anti-rollback(FR-BL-008/FR-AB-008) 구현 — ADR-007 추가(앞 서명 헤더, MCUboot 정석). 서명 이미지 `[헤더0x200][코드][서명64]`, ECDSA가 (헤더+코드) 덮음, 앱 링커 origin +0x200, 부트로더 점프/검증 +0x200·ECDSA 후 헤더 fw_version을 메타 CONFIRMED 버전과 비교해 다운그레이드 거부. sign_firmware.py `--version`, write_pending이 헤더 fw_version 기록. 구현은 헤더 subset(magic/fw_version/target_ecu_id) — header_version/hw_id/hash·Manifest 출처는 후속. 단위테스트 anti_rollback 6 신규(총 54), CRC 기준선·on-target 부팅 검증 한계는 ADR-007 기록 |
